...
| `g_hex_neighbor_delta_x_by_parity` / `g_hex_neighbor_delta_y_by_parity` | Road, long-wall, map decode, AI, pathfinding, and editor erase paths index these by y-parity and neighbor direction to visit adjacent hex/staggered-map tiles. | Shared hex-neighbor delta tables. |
| `g_battle_feature_values` | `Resource_Able`, `Clear_Forest_Or_Resource`, `Calc_City_Resource`, and `Map_To_Battle_Army` index this by `LandTile.battle_resource_or_feature_id` to gate resources, play clear effects, and grant battle bonuses. | Battle resource/terrain-feature value table. |
...
| `g_editor_selected_battle_feature_set` | Tool mode `4` uses `0` as random-set selection and positive values `1..5` to choose feature id ranges before writing `LandTile.battle_resource_or_feature_id`. | Selected battle-resource/feature brush set. |
| `g_editor_battle_feature_variant_mode` | Tool mode `4` switches random ranges within the selected feature set; modes `0..3` choose broad, narrow, and offset ranges. | Battle-feature random variant mode. |
| `g_editor_battle_feature_base_id_slots` / `g_editor_battle_feature_custom_base_id` | Tool mode `4` combines these low-byte base ids with random offsets to produce the tile's battle resource/feature id; selected feature sets step through the slot table by 4 bytes. | Battle-feature brush base id slots. |
...
| `g_ground_editor_selected_row` | `Before_Edit_Ground` uses this as the row source for all `GroundDef_0x24` controls and as the stride-scaled index into `g_ground_defs`. | Selected ground/terrain definition row. |
| `g_ground_defs_edit_backup` | `Before_Edit_Ground` allocates `0x21c` bytes and copies all 15 `GroundDef_0x24` records before binding the editor controls. | Ground definition editor undo/cancel backup. |
| `g_ground_default_file_exists` | `Before_Edit_Ground` appends `GROUND.DAT` to the data path and sets this flag from the file-open result. | Default ground definition file availability flag. |
...
| `Save_IMG_Flag` | Trace string `Save_IMG_Flag`; copies live flag pixels into the edit backups, writes a two-byte count (`100`) plus 100 live `0x100`-byte flag blocks to `FLAG.IMG`, then reloads the IMG resource bank. | Empire flag IMG persistence. |
| `MLP_Edit_Empire_Flag` | Trace string `MLP_Edit_Empire_Flag`; maps mouse position/color-channel selectors to RGB levels, then writes a 16-bit pixel into `g_flag_img_bank[g_edit_flag_index] + 4 + (x + y * 14) * 2`. | Empire flag pixel edit handler. |
...
| `4` | Left-press chooses an id from `g_editor_selected_battle_feature_set`, `g_editor_battle_feature_variant_mode`, and feature-base tables, then writes `LandTile.battle_resource_or_feature_id`; right-press clears `+0x16`. | Battle resource/terrain-feature brush. |
| `5` | Left/right press edit road/bridge bytes `+0x13..+0x15`, battle feature `+0x16`, long wall `+0x24`, and alternate terrain bytes, then calls road/long-wall/map decode over neighboring tiles. | Overlay/road/bridge/long-wall detail brush. |
| `6` | `MLR_Edit_GameMap` validates `g_editor_selected_city_resource_id` through `CityResourceDef.terrain_compatibility_by_kind`, `placement_or_resource_class`, and `g_battle_feature_values`, then assigns the resource id and initial stockpile; right-press clears tile `+0x17/+0xf8`. | City resource/feature brush. |
